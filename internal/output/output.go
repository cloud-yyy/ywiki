...
func HandleError(w io.Writer, err error) int {
	if err == nil {
		return wikierrors.ExitSuccess
	}

	genericExitErr := &wikierrors.ExitError{
	...
	if invalidFieldErr, ok := errors.AsType[*wikierrors.InvalidFieldError](err); ok {
		if IsJSON() {
			if data, jsonErr := invalidFieldErr.JSONError(); jsonErr == nil {
				fmt.Fprintln(JSONErrorWriter(w), string(data))
			}
		} else {
			wikierrors.PrintHuman(w, &invalidFieldErr.ExitError, ColorsEnabled())
		}
		return invalidFieldErr.ExitCode
	}

	if exitErr, ok := errors.AsType[*wikierrors.ExitError](err); ok {
		if IsJSON() {
			...
			wikierrors.PrintHuman(w, exitErr, ColorsEnabled())
		}
	...
	wikierrors.PrintHuman(w, genericExitErr, ColorsEnabled())
	return genericExitErr.ExitCode
}
