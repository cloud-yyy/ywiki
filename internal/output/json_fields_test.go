...
func TestValidateFields_Invalid(t *testing.T) {
	...
	var fieldErr *wikierrors.InvalidFieldError
	ok := false
	fe := &wikierrors.InvalidFieldError{}
	if errors.As(err, &fe) {
		fieldErr = fe
		ok = true
	}
...
func TestInvalidFieldError_JSONError(t *testing.T) {
	err := wikierrors.NewInvalidFieldError("badfield", []string{"key", "summary"})
