...
func TestHandleError_ExitError(t *testing.T) {
	...
	err := wikierrors.NewAuthError("auth failed", "login again")
...
func TestHandleError_ExitError_JSON(t *testing.T) {
	...
	err := wikierrors.NewNotFoundError("issue not found", "check the key")
	code := output.HandleError(&buf, err)
...
func TestHandleError_InvalidFieldError_JSON(t *testing.T) {
	...
	err := wikierrors.NewInvalidFieldError("bogus", []string{"key", "summary"})
	code := output.HandleError(&buf, err)
...
func TestHandleError_GenericError(t *testing.T) {
	...
	if code != wikierrors.ExitUserError {
		t.Errorf("HandleError(generic) = %d, want %d", code, wikierrors.ExitUserError)
	}
}
...
func TestHandleError_GenericError_JSON(t *testing.T) {
	...
	if code != wikierrors.ExitUserError {
		t.Errorf("HandleError(generic JSON) = %d, want %d", code, wikierrors.ExitUserError)
	}
