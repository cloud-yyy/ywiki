...
func TestExitCodes(t *testing.T) {
	tests := []struct {
		...
		{"ExitSuccess", wikierrors.ExitSuccess, 0},
		{"ExitUserError", wikierrors.ExitUserError, 1},
		{"ExitAuthError", wikierrors.ExitAuthError, 3},
		{"ExitNotFound", wikierrors.ExitNotFound, 4},
		{"ExitRateLimited", wikierrors.ExitRateLimited, 5},
		{"ExitInterrupted", wikierrors.ExitInterrupted, 130},
	}
...
func TestExitError_JSONError(t *testing.T) {
	t.Run("with suggestion", func(t *testing.T) {
		err := &wikierrors.ExitError{
			...
			Suggestion: "Run: ywiki auth login",
		}
		...
		if result["suggestion"] != "Run: ywiki auth login" {
			t.Errorf("suggestion = %q, want %q", result["suggestion"], "Run: ywiki auth login")
		}
	})
...
func TestNewAuthError(t *testing.T) {
	err := wikierrors.NewAuthError("not authenticated", "Run: ywiki auth login")
	...
	if err.Suggestion != "Run: ywiki auth login" {
		t.Errorf("Suggestion = %q, want %q", err.Suggestion, "Run: ywiki auth login")
	}
}

func TestNewNotFoundError(t *testing.T) {
	err := wikierrors.NewNotFoundError("issue not found", "Check the issue key")
...
func TestNewUserError(t *testing.T) {
	err := wikierrors.NewUserError("invalid flag", "Use --help for usage")
...
func TestNewRateLimitedError(t *testing.T) {
	err := wikierrors.NewRateLimitedError("rate limit exceeded", "Wait and retry")
...
func TestErrorsAs(t *testing.T) {
	original := wikierrors.NewAuthError("auth failed", "login again")
	wrapped := fmt.Errorf("command failed: %w", original)

	var target *wikierrors.ExitError
...
func TestPrintHuman_NoColors(t *testing.T) {
	...
	wikierrors.PrintHuman(&buf, err, false)
...
func TestPrintHuman_WithColors(t *testing.T) {
	...
	wikierrors.PrintHuman(&bufColor, err, true)
	wikierrors.PrintHuman(&bufPlain, err, false)
